...
использовался **chromadb.PersistentClient**, так как данные хранятся на диске и при перезапуске они не пропадут, а будут хранится на диске. Если бы использовался **chromadb.Client**, то при перезапуске надо было 
бы заново заполнять БД, так как в таком случаи данные хранятся в RAM и при отсановке работы python-файла данные бы стерлись.

## Embedding Model
Модель была выбрана **multilingual-e5-small**. 

<img width="1494" height="641" alt="image" src="https://github.com/user-attachments/assets/17fb56da-90e3-4c45-adae-a85c61d815ed" />

Проаназилировав **MTEB Benchmark** были сделаны следующие выводы: показатель Retrivial составляет 63.19, но при этом модель занимает в памяти 449 MB, 
...
для поставленной задачи хорошо подходит. Поэтому в качестве inference использовалась llama-cpp-python.
...
2. Перейдя в папку проекта пропишите в консоли ```docker-compose up --build``` 
